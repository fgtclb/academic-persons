...
**A phone link target carries no spaces.** The stored number was written into
:html:`href="tel:…"` unchanged, so a number stored as ``+49 6241 509 123``
produced a target with spaces in it. The spaces are now removed from the
target, both in the contract field partial and in
:file:`Resources/Private/Templates/Profile/Detail.html`, which builds a target
of its own. The visible link text keeps the stored spelling.
...
:file:`Resources/Private/Partials/Profile/Contract/Field.html` or
:file:`Resources/Private/Templates/Profile/Detail.html` keeps its own output
and profits from neither correction until it drops its copy. The arguments the
partial is called with are unchanged, so an override of
...
profile list, card or selection element, or in the profile detail view.
