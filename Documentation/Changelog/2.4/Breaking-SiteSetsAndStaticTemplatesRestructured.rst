...
Site sets arrived in TYPO3 v13.1. On **TYPO3 v12 they do not exist**, so
everything below that names a site set applies to v13 only, and on v12 the
static template and the page field :guilabel:`Page TSconfig` are the whole
mechanism. The set definitions this extension ships are simply never read there.
...
The entry each content element has in the new content element wizard moved into
the page TSconfig of its component as well, together with the
:typoscript:`mod.wizards.newContentElement.wizardItems.academic.show` entry that
TYPO3 v12 gates it on. Both arrive together or not at all.
...
ungrouped list. On TYPO3 v12 this affects every installation, because there is
no site set there.
...
TSconfig of its component is included, through the page field
:guilabel:`Page TSconfig` or — on TYPO3 v13 — through the site set. This affects
every installation: before this change the extension hid nothing.
...
..  warning::
    ...
    This holds on TYPO3 v12 and v13 alike. Include the page TSconfig of the
    components on every page tree that holds such records, and do it before
    editing them.
...
This is the migration step for **every** installation on TYPO3 v12. On TYPO3
v13, sites that use a site set instead need no migration — but they must not use
...
On TYPO3 v13 a site configuration may name the new component sets instead of the
aggregate:
