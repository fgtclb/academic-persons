...
on the 3.x line, making the person tables workspace aware added an index over
:sql:`t3ver_oid`, which gave the PostgreSQL planner a way to satisfy the
:sql:`t3ver_oid = 0` constraint Extbase adds for a workspace aware table, and
reversed the result of exactly these queries. This line does not add that
index, but an index an installation adds itself has the same effect.
