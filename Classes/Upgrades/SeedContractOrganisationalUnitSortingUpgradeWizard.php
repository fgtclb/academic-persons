 ...
 * orders by `organisational_unit_sorting` from 3.0 on, and every contract that
 ...
 *
 * **Every row carrying an organisational unit is numbered**, deleted ones and
 * workspace versions included. The column is a rank within the unit, and ordering
 * only ever compares rows: leaving a row at `0` would put it above every numbered
 * sibling the moment it is restored or published. Numbering by the parent uid the
 * row carries is the same grouping `RelationHandler` itself uses when it writes
 * the column, so the relative order of any set of rows it lists - a live list, a
 * workspace list - is the order this wizard seeds.
 */
#[UpgradeWizard('academicPersons_seedContractOrganisationalUnitSorting')]
final readonly class SeedContractOrganisationalUnitSortingUpgradeWizard implements UpgradeWizardInterface
{
    ...
    public function getDescription(): string
    {
        return 'Contracts are inline children of their profile and of their organisational unit, and'
            ...
            . ' column of its own from 3.0 on. This wizard fills it with the order the organisational'
