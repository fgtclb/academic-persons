...
/**
 * Covers the three public methods of `PhoneNumberRepository`.
 *
 ...
 * `ContractController`/`PhoneNumberController`), `ProfileFactory` and the
 ...
 */
final class PhoneNumberRepositoryTest extends AbstractAcademicPersonsTestCase
{
    private const FIXTURE = __DIR__ . '/Fixtures/PhoneNumberRepositoryTest/phoneNumbers.csv';

    private function subject(): PhoneNumberRepository
    {
        return $this->get(PhoneNumberRepository::class);
    }

    /**
     * @param QueryResultInterface<int, PhoneNumber> $result
    ...
    public function findByContractIncludingHiddenMapsTheRecordValues(): void
    {
        ...
        $numbers = [];
        foreach ($this->subject()->findByContractIncludingHidden(1) as $phoneNumber) {
            $numbers[] = $phoneNumber->getPhoneNumber();
        }

        $this->assertSame(
            ['+49 228 000002', '+49 221 000003', '+49 30 000001', '+49 365 000007'],
            $numbers,
        );
    }
    ...
    /**
     * Contract 3 exists and simply has no phone numbers - the case a profile without a phone
     * number produces, which must be an empty result rather than everything.
     */
    #[Test]
    ...
    /**
     * `contract` is `int unsigned DEFAULT '0' NOT NULL`, so a phone number that was never
    ...
    public function findByUidIncludingHiddenReturnsAVisibleRecord(): void
    {
        ...
        $phoneNumber = $this->subject()->findByUidIncludingHidden(1);

        $this->assertInstanceOf(PhoneNumber::class, $phoneNumber);
        $this->assertSame(1, $phoneNumber->getUid());
        $this->assertSame('+49 30 000001', $phoneNumber->getPhoneNumber());
    }

    /**
     * Extbase argument mapping respects the enable fields, so without this method the frontend
     * editing UI could list a hidden phone number but not open it for editing.
     */
    #[Test]
    public function findByUidIncludingHiddenReturnsAHiddenRecord(): void
    {
        ...
        $phoneNumber = $this->subject()->findByUidIncludingHidden(2);

        $this->assertInstanceOf(PhoneNumber::class, $phoneNumber);
        $this->assertSame(2, $phoneNumber->getUid());
        $this->assertSame('+49 228 000002', $phoneNumber->getPhoneNumber());
    }
    ...
    public function findByUidIncludingHiddenIgnoresTheStoragePage(): void
    {
        ...
        $phoneNumber = $this->subject()->findByUidIncludingHidden(7);

        $this->assertInstanceOf(PhoneNumber::class, $phoneNumber);
        $this->assertSame('+49 365 000007', $phoneNumber->getPhoneNumber());
    }
