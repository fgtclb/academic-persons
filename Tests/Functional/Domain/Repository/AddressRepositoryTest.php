...
/**
 * Covers the three public methods of `AddressRepository`.
 *
 ...
 * `ContractController`/`PhysicalAddressController`) and `ProfileFactory` can list and resolve a
 * record an editor just toggled off. A regression there does not raise anything - the record
 * simply stops appearing in the edit form, and the editor can never switch it back on.
 *
 ...
 */
final class AddressRepositoryTest extends AbstractAcademicPersonsTestCase
{
    private const FIXTURE = __DIR__ . '/Fixtures/AddressRepositoryTest/addresses.csv';

    private function subject(): AddressRepository
    {
        return $this->get(AddressRepository::class);
    }
     ...
     * On this branch PostgreSQL cannot either: the table is not workspace aware, so there is
     * no `t3ver_oid` index for the planner to prefer over a scan in insertion order, and the
     * test stays green on `-d postgres` without the tiebreaker as well. On the 3.x line, where
     * that index exists, the same test is red there. The assertion pins the contract here.
     */
    #[Test]
    public function findByContractIncludingHiddenBreaksEqualSortingTiesByUid(): void
    {
        $this->importCSVDataSet(__DIR__ . '/Fixtures/AddressRepositoryTest/equalSorting.csv');
    ...
    public function findByContractIncludingHiddenMapsTheRecordValues(): void
    {
        ...
        $cities = [];
        foreach ($this->subject()->findByContractIncludingHidden(1) as $address) {
            $cities[] = $address->getCity();
        }

        $this->assertSame(['Bonn', 'Cologne', 'Berlin', 'Gera'], $cities);
    }
    ...
    /**
     * Contract 3 exists and simply has no addresses - the case a profile without a postal
    ...
    /**
     * `contract` is `int unsigned DEFAULT '0' NOT NULL`, so an address that was never assigned
     * to a contract carries 0 - and 0 is also what an unresolved argument degrades to. The
     * method treats it as an ordinary value and returns exactly those orphaned records, which
     * is worth pinning down: it is neither a wildcard nor an error.
     */
    #[Test]
    ...
    public function findByUidIncludingHiddenReturnsAVisibleRecord(): void
    {
        ...
        $address = $this->subject()->findByUidIncludingHidden(1);

        $this->assertInstanceOf(Address::class, $address);
        $this->assertSame(1, $address->getUid());
     ...
     * editing UI could list a hidden address but not open it for editing.
     */
    #[Test]
    public function findByUidIncludingHiddenReturnsAHiddenRecord(): void
    {
        ...
        $address = $this->subject()->findByUidIncludingHidden(2);

        $this->assertInstanceOf(Address::class, $address);
        $this->assertSame(2, $address->getUid());
        $this->assertSame('Bonn', $address->getCity());
    }
    ...
    public function findByUidIncludingHiddenIgnoresTheStoragePage(): void
    {
        ...
        $address = $this->subject()->findByUidIncludingHidden(7);

        $this->assertInstanceOf(Address::class, $address);
        $this->assertSame('Gera', $address->getCity());
    }
